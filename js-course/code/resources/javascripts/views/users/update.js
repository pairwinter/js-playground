...
(function(view){
    view.user=function(){};
    view.user.update = function(){};
    view.user.update.initPage=function(context){
        new EB_Common.toolTip({orientation:'element',arrows:true,tipCls:'b-tooltip-info-map',tipAttach:'map',tipMaxWidth:150});
        var doDelete=this.doDelete;
        EB_Common.validation.validate("validation-form",{rules: {}});

//        if(contactException!=""){
//            EB_Common.dialog.alert(i18n['user.update.contact.removed']);
//        }
        ...
        $("#add_role").click(function(){
            ...
            if(roleTemplete=="role_account_admin"){
                var newTr='<tr id="tr'+trId+'">'
                    ...
                    +'<td><a onClick=EB_View.user.update.initPage.doDelete("'+trId+'") class="icn_trash_16" href="javascript:void(0)"></a></td></tr>';
                ...
                var newTr='<tr id="tr'+trId+'">'
                    ...
                    +'<td>'+groupNameStr+'</td><input name="groups" type="hidden" value="'+groupIdStr+'">'
                    +'<td><a onClick=EB_View.user.update.initPage.doDelete("'+trId+'") class="icn_trash_16" href="javascript:void(0)"></a></td></tr>';
                $("#roleTable").append(newTr);
            }else{
...
//                            }
        ...
        $("#btn_send_forgot_password_email").click(function(){
            EB_Common.Ajax.post("/users/password/mail",{id:userId,_method:'PUT'},function(data){
                if(data.status!=""){
                    EB_Common.dialog.alert(i18n['user.list.mail.success']);
                }else{
                    EB_Common.dialog.alert(i18n['user.list.mail.fail']);
    ...
    view.user.update.initPage.doDelete=function(id){
        $("tr[id=tr"+id+"]").remove();
        var trId=id.split("-");
