...
SWFUpload.prototype.initSWFUpload = function(settings) {
    try {
        this.customSettings = {}; // A container where developers can place their own settings associated with this
        // instance.
...
SWFUpload.prototype.initSettings = function() {
    ...
    if (!this.settings.preserve_relative_urls) {
        // this.settings.flash_url = SWFUpload.completeURL(this.settings.flash_url); // Don't need to do this one since
        // flash doesn't look at it
...
SWFUpload.prototype.loadFlash = function() {
    ...
    tempParent.innerHTML = this.getFlashHTML(); // Using innerHTML is non-standard but the only sensible way to
...
SWFUpload.prototype.getFlashVars = function() {
    ...
    return [ "movieName=", encodeURIComponent(this.movieName), "&amp;uploadURL=",
            ...
            "&amp;filePostName=", encodeURIComponent(this.settings.file_post_name), "&amp;fileTypes=",
...
// Returns true if everything was destroyed. Returns a false if a failure occurs leaving SWFUpload in an inconsistant
// state.
...
SWFUpload.prototype.destroy = function() {
    try {
        ...
        if (movieElement && typeof (movieElement.CallFunction) === "unknown") { // We only want to do this in IE
            // Loop through all the movie's properties and remove all function references (DOM/JS IE 6/7 memory leak
            // workaround)
 ...
 */
// Public: (Deprecated) addSetting adds a setting value. If the value given is undefined or null then the default_value
// is used.
...
/***********************************************************************************************************************
 * Settings methods These methods change the SWFUpload settings. SWFUpload settings should not be changed directly on
 * the settings object since many of the settings need to be passed to Flash in order to take effect.
 * ***********************
 */
 ...
 * 
 * Because the ExternalInterface library is buggy the event calls are added to a queue and the queue then executed by a
 * setTimeout. This ensures that events are executed in a determinate order and that the ExternalInterface bugs are
 * avoided.
 **********************************************************************************************************************/
...
// Private: unescapeFileParams is part of a workaround for a flash bug where objects passed through ExternalInterface
// cannot have
...
/*
 * Called after the file dialog has closed and the selected files have been queued. You could call startUpload here if
 * you want the queued files to begin uploading immediately.
 */
...
/***********************************************************************************************************************
 * Debug Console The debug console is a self contained, in page location for debug message to be sent. The Debug Console
 * adds itself to the body if necessary.
 * 
 * The console is automatically scrolled as messages appear.
 * 
 * If you are using your own debug handler or when you deploy to production and have debug disabled you can remove these
 * functions to reduce the file size and complexity.
 **********************************************************************************************************************/
